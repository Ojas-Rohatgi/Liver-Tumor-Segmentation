...
OUTPUT_DATA_DIR = Path.cwd() / "data_2_5d_liver"

TARGET_LABEL = 1  # 1 for Liver. (We treat label 2 (tumor) as 1 (liver) here)
...
def resample_and_normalize(volume, original_spacing, target_shape=RESIZE_SHAPE):
    ...
    """
    # This script assumes resizing, but we set target_shape = (512, 512)
    # If your scans are not 512x512, this will resize them.
...
def process_scan(vol_path, mask_path, img_out_dir, msk_out_dir, context_slices):
    ...
    try:
        ...
        # Using original shape, so this just normalizes
        ...
        # --- IMPORTANT ---
        # For LIVER model, we treat tumors (label 2) as part of the liver (label 1)
        # So, our target mask is anything > 0
        if TARGET_LABEL == 1:
            mask_data_target = (mask_data_resampled > 0).astype(np.uint8)
        else:
            # This logic would be for a tumor-only script
        ...
        for i in range(num_slices):
            # Only save slices that contain the liver
...
def main():
    print("Starting 2.5D data preparation for LIVER model...")
    ...
    print(f"Total 2.5D slices saved: {total_slices_saved}")
